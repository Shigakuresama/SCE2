...
function getActiveSectionTitle() {
    const activeElement = document.querySelector('.sections-menu-item.active .sections-menu-item__title');
    if (activeElement) {
...
function waitForTextContent(selector, timeout = 10000, minLength = 1) {
    return new Promise((resolve, reject) => {
        const checkText = () => {
            const element = document.querySelector(selector);
...
// SUBMIT MODE
// ==========================================
async function performSubmit(jobData) {
    const helper = new SCEHelper();
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
    ...
    switch (message.action) {
        case 'SCRAPE_PROPERTY':
            performScrape(message.data)
                .then(sendResponse)
                .catch(err => sendResponse({ success: false, error: err.message }));
            return true; // Keep channel open for async
        case 'SUBMIT_APPLICATION':
            performSubmit(message.data)
                .then(sendResponse)
                .catch(err => sendResponse({ success: false, error: err.message }));
            return true; // Keep channel open for async
